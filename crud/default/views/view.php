...
<?php $label = StringHelper::basename($generator->modelClass); ?>
    <h3><?= "<?= \$model->" . $generator->getModelNameAttribute($generator->modelClass) . " ?>" ?></h3>
...
foreach ($generator->getTableSchema()->columns as $column) {
    $format = str_replace("\n","\n\t\t\t",$generator->attributeFormat($column));
    if ($format === false) {
        continue;
    } else {
        echo "\t\t\t" . $format . ",\n";
...
?>
    ...
    <?= "<?= " ?>Html::a('<span class="glyphicon glyphicon-trash"></span> Delete', ['delete', <?= $urlParams ?>],
    [
...
foreach ($generator->getModelRelations($generator->modelClass, ['has_many']) as $name => $relation) {
    ...
    if ($relation->via !== null) {
        ...
        $addButton = "  <?= \\yii\\helpers\\Html::a('<span class=\"glyphicon glyphicon-link\"></span> Attach "
            . Inflector::singularize(Inflector::camel2words($name))
            . "', ['" . $generator->createRelationRoute($pivotRelation, 'create') . "', '"
            . Inflector::singularize($pivotName) . "'=>['" . key($pivotRelation->link) 
            . "'=>\$model->{$model->primaryKey()[0]}]], ['class'=>'btn btn-info btn-xs']) ?>\n";
    } else {
        $addButton = '';
    }
    ...
    echo "\t\t<?= \\yii\\helpers\\Html::a('<span class=\"glyphicon glyphicon-list\"></span> List All "
        . Inflector::camel2words($name)
        . "', ['" . $generator->createRelationRoute($relation, 'index')
        . "'], ['class'=>'btn text-muted btn-xs'] ) ?>\n";
    // TODO: support multiple PKs, VarDumper?
    echo "\t\t<?= \\yii\\helpers\\Html::a('<span class=\"glyphicon glyphicon-plus\"></span> New "
        . Inflector::singularize(Inflector::camel2words($name))
        . "', ['" . $generator->createRelationRoute($relation, 'create') . "', '" 
        .  Inflector::singularize($name) . "'=>['" . key($relation->link) 
        . "'=>\$model->" . $model->primaryKey()[0] . "]], ['class'=>'btn btn-success btn-xs']) ?>\n";
    echo $addButton;
